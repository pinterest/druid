...
{
  private static final Logger log = new Logger(CollectSetUtil.class);
  ...
  public static void addCollectionWithLimit(Set<Object> set, Collection<?> valueCollection, int limit)
  {
    ...
    log.info("limit" + limit + "--set size:" + set.size());
    log.info("value size:" + valueCollection.size());
    ...
    if (limit >= 0 && valSize > limit - set.size()) {
      ...
      log.info("Add all Collection Value" + valueCollection);
  ...
  public static THashSet<Object> flatten(Collection<?> input)
  {
    ...
    log.info("In HashSet:" + input.toString());
